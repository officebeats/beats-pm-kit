...
pipeline: [deep-interview, engineering-planner, autopilot]
next-skill: engineering-planner
...
- User wants to explore options or brainstorm -- use `engineering-planner` skill instead
...
- Once ready, do **not** bridge into `engineering-planner`, `autopilot`, `ralph`, or `team`. Instead run:
...
1. **Ralplan → Autopilot (Recommended)**
   ...
   - Action: Invoke `Skill("oh-my-claudecode:engineering-planner")` with `--consensus --direct` flags and the spec file path as context. The `--direct` flag skips the engineering-planner skill's interview phase (the deep interview already gathered requirements), while `--consensus` triggers the Planner/Architect/Critic loop. When consensus completes and produces a plan in `.omc/plans/`, invoke `Skill("oh-my-claudecode:autopilot")` with the consensus plan as Phase 0+1 output — autopilot skips both Expansion and Planning, starting directly at Phase 2 (Execution).
   - Pipeline: `deep-interview spec → engineering-planner --consensus --direct → autopilot execution`
...
- [ ] If 3-stage pipeline selected: engineering-planner --consensus --direct invoked, then autopilot with consensus plan
...
/deep-interview "vague idea"
  ...
  → /engineering-planner --consensus --direct (spec as input, skip interview)
...
**The engineering-planner skill receives the spec with `--consensus --direct` flags** because the deep interview already did the requirements gathering. The `--direct` flag (supported by the engineering-planner skill, which ralplan aliases) skips the interview phase and goes straight to Planner → Architect → Critic consensus. The consensus plan includes:
