---
title: "Watchdog Advisor Review Priorities"
type: "rules"
source: "beats-pm-kit"
---

# 🛡️ Watchdog — Advisor Review Priorities
...
- Drafting is fine; SENDING is not. Flag any attempt to send email, post to Slack/Teams, or transition Jira issues unless the user explicitly asked for that exact message/action this turn.
- Confluence/Jira/ADO writes: confirm the user asked for the write, not just a draft.

## Privacy (blocker)
- PHI or member/patient-level data leaving the vault (into web searches, external MCP writes, or shared artifacts).
