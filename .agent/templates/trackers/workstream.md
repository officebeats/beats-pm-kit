# [Workstream Title]

> Status: [Today/Next/Later/Follow Up/Triage]
> Owner: [Owner]
> Last reviewed: [YYYY-MM-DD]
> Agent Refs: [Task Master IDs, Trello cards, Jira/Confluence links, transcript/chat evidence]
> Job: When [situation], I want to [motivation], so I can [outcome]
> Outcome metric: [measurable signal that this workstream is done/winning]

## Latest Outcomes

- [Result, decision, or current state]
  - Evidence: [workstream title; started from initial source on date; latest progress from latest source on date]

## Completed Outcomes

- [Completed result or checked checklist item]
  - Completed: [date/source]

## Open Items

- [Owner] - [action] by [date/gate]
  - Source: [Outlook/Teams/Slack/Calendar/transcript/Quill/Granola/local packet]

## Recommended Next 3

- [Action 1]
- [Action 2]
- [Action 3]

## Evidence Log

- [YYYY-MM-DD] [source] - [short evidence note]

## Linked Tasks

- [Task title](../tasks/TASK-ID.md) - [status]
