    ...
    for name in CONTENT_ROOTS:
        ...
        for path in lane.rglob("*"):
            ...
            if any(part.startswith(".") or part in SKIP_DIRS for part in relative.parts):
                continue
            if relative == LABEL_MAP or BACKUP_PREFIX in relative.parents:
                continue
            files.append(path)
...
def render_map(labels: Sequence[NoteLabel]) -> str:
    ...
    for item in labels:
        if item.path.startswith(TASKS_PREFIX):
            rows.append(
                f"| [{table_value(item.label)}]({map_link(item.path)}) | "
                f"`{table_value(item.agent_ref)}` | `{table_value(item.path)}` |"
            )
    rows.extend(["", "## Full local catalog", "", "| Human label | Area | Note |", "|:---|:---|:---|"])
    for item in labels:
        rows.append(
            f"| [{table_value(item.label)}]({map_link(item.path)}) | "
            f"{table_value(item.path.split('/', 1)[0])} | `{table_value(item.path)}` |"
        )
    return "\n".join(rows) + "\n"
