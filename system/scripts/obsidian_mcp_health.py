...
def probe_endpoint(url: str, api_key: str, timeout: float) -> tuple[bool, str]:
    ...
    if parsed.scheme == "https" and parsed.hostname not in {"127.0.0.1", "::1", "localhost"}:
        return False, "non_loopback_https_refused"
    context = ssl.create_default_context()
    # Obsidian Local REST API uses a self-signed localhost certificate. This
    # health probe is restricted to loopback HTTPS and never writes to Obsidian.
