...
class TestTaskDisplay(unittest.TestCase):
    def test_task_file_provenance_uses_titles_and_sources(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            task_path = Path(tmpdir) / "PARU-030.md"
            task_path.write_text(
                "\n".join(
                    [
                        "# PARU-030 — Stand up Synapse API intake",
                        "",
                        "> **Created:** 2026-04-23",
            ...
            self.assertEqual(provenance.display_title, "Stand up Synapse API intake")
            ...
            self.assertEqual(
                ...
                "Stand up Synapse API intake; started from Operational strategy sync on 2026-04-23; latest progress from Partner Slack delta on 2026-06-26.",
            )

    def test_scrubber_replaces_known_and_unknown_hard_ids(self):
        text = "PARU-030 depends on INIT-292 and DUI2-75."

        scrubbed = task_display.scrub_visible_refs(text, {"PARU-030": "Stand up Synapse API intake"})

        self.assertIn("Stand up Synapse API intake", scrubbed)
        self.assertNotIn("PARU-030", scrubbed)
