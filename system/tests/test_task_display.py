    ...
    def test_task_file_provenance_uses_titles_and_sources(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            task_path.write_text(
                "\n".join(
                    [
                        "# PLAN-030 — Stand up partner API intake",
                        "",
                        "> **Created:** 2026-04-23",
            ...
            self.assertEqual(provenance.display_title, "Stand up partner API intake")
            ...
            self.assertEqual(
                ...
                "Stand up partner API intake; started from Operational strategy sync on 2026-04-23; latest progress from Partner Slack delta on 2026-06-26.",
            )

    def test_scrubber_replaces_known_and_unknown_hard_ids(self):
        text = "PLAN-030 depends on INIT-292 and OPS2-75."

        scrubbed = task_display.scrub_visible_refs(text, {"PLAN-030": "Stand up partner API intake"})

        self.assertIn("Stand up partner API intake", scrubbed)
        self.assertNotIn("PLAN-030", scrubbed)
        self.assertNotIn("INIT-292", scrubbed)
        self.assertNotIn("OPS2-75", scrubbed)
