...
""",
    ...
    def test_parses_legacy_id_heading_without_renaming_file(self):
        root = self.make_root()
        path = root / "5. Trackers" / "tasks" / "PARU-014.md"
        path.write_text(
            "# PARU-014 — Review recommendation work\n\n> **Status:** Active\n> **Owner:** Owner\n",
            encoding="utf-8",
        )
        ...
        self.assertEqual(task.task_id, "PARU-014")
        self.assertEqual(task.title, "Review recommendation work")
        self.assertEqual(task.path.name, "PARU-014.md")
