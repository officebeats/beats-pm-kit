...
class TestTrelloBridgeBoundedSync(unittest.TestCase):
    def classification(self, local_id: str, name: str, path: str = "5. Trackers/tasks/PARU-001.md"):
        return {
    ...
    def test_filter_classifications_matches_task_id(self):
        rows = [
            self.classification("PARU-001", "Guideline exception intake"),
            self.classification("PARU-002", "Developer portal path"),
        ]

        filtered = trello_bridge.filter_classifications(rows, only_tasks=["PARU-002"])

        self.assertEqual(len(filtered), 1)
        self.assertEqual(filtered[0]["local_id"], "PARU-002")
