...
class TestCodexSkillAdapters(unittest.TestCase):
    ...
    def test_sync_prunes_stale_generated_beats_skills_only(self):
        """Old generated beats-* adapters should not linger in the Codex picker."""
        with tempfile.TemporaryDirectory() as tmpdir:
            skills_dir = Path(tmpdir)
            stale = skills_dir / "beats-test-qa-reasoning"
            stale.mkdir()
            (stale / "SKILL.md").write_text(
                "# stale\n\n> Auto-generated by `system/scripts/sync_codex_skill_adapters.py`.\n",
                encoding="utf-8",
            )
            custom = skills_dir / "beats-private"
            custom.mkdir()
            (custom / "SKILL.md").write_text("# private user skill\n", encoding="utf-8")
    ...
    def test_generated_vacuum_skill_does_not_claim_archive_command(self):
        """The /vacuum adapter should not advertise the separate /archive workflow."""
        with tempfile.TemporaryDirectory() as tmpdir:
            sync_codex_skill_adapters.sync_promoted_skills(output_dir=tmpdir, root=ROOT_DIR)
            skill_md = Path(tmpdir) / "beats-vacuum" / "SKILL.md"
            content = skill_md.read_text(encoding="utf-8")
