...
class TestContextRouter(unittest.TestCase):
    def make_root(self) -> tempfile.TemporaryDirectory:
        ...
        (root / "5. Trackers" / "tasks" / "PLAN-009.md").write_text(
    ...
    def test_build_index_covers_supported_local_sources(self):
        with self.make_root() as tmpdir:
            ...
            self.assertIn("5. Trackers/tasks/PLAN-009.md", paths)
    ...
    def test_cache_invalidates_when_file_changes(self):
        with self.make_root() as tmpdir:
            ...
            task_path = root / "5. Trackers" / "tasks" / "PLAN-009.md"
            old_hash = next(item["sha256"] for item in first["files"] if item["path"].endswith("PLAN-009.md"))
            ...
            new_hash = next(item["sha256"] for item in second["files"] if item["path"].endswith("PLAN-009.md"))
    ...
    def test_write_wiki_uses_ignored_cache_shape(self):
        with self.make_root() as tmpdir:
            ...
            self.assertIn("PLAN-009.md", wiki_path.read_text(encoding="utf-8"))
