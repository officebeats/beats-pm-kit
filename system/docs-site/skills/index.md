...
> Auto-generated index of all agent skills in the Antigravity Kit.

Run `npm run generate` to refresh this page.

::: tip
Each skill is a self-contained capability module with its own SKILL.md, templates, and scripts.
:::
...
| `cover-letter-writer` | Generates tailored, narrative-driven cover letters based on Resume and Job Description. |
...
| `daily-synth` | Daily briefing and planning. |
...
| `engineering-collab` | Architecture, tech debt, and spike management. |
...
| `frontend-engineer` | Build, test, and style premium UI components with WCAG compliance. |
...
| `meeting-synth` | "Synthesize a single meeting transcript into structured action items, decisions, and follow-ups. Use when processing a meeting recording, cleaning up transcript notes, or extracting commitments from a conversation." |
...
| `okr-manager` | "Brainstorm team-level OKRs aligned with company objectives — qualitative objectives with measurable key results. Use when setting quarterly OKRs, aligning team goals with company strategy, drafting objectives, or learning how to write effective OKRs." |
...
| `prd-author` | "Author comprehensive Product Requirements Documents (PRDs) using a strict 8-section unified template. Use when the user requests /prd, needs to draft new product specifications, or outlines a new feature. Do NOT use this for meeting summaries or task trackers." |
...
| `release-manager` | "Generate user-facing release notes from tickets, PRDs, or changelogs. Creates clear, engaging summaries organized by category (new features, improvements, fixes). Use when writing release notes, creating changelogs, announcing product updates, or summarizing what shipped." |
...
| `sales-automator` | AI agent team for sales intelligence, lead analysis, and outreach automation. |
...
| `shadcn-ui` | Expert UI engineering capabilities using shadcn/ui. Integrates accessible, tailwind-styled components. |
...
| `stakeholder-mgr` | Manage relationships, 1:1s, and stakeholder communication. |
| `startup-analyst` | AI consulting agent providing startup analysis, competitive research, and business strategy. |
...
| `startup-financial-modeling` | AI finance team for startup financial modeling, investment analysis, and projections. |
| `stitch` | Official interface for the Stitch AI UI/UX design tool via MCP. |
...
| `system-validation` | Validates the three-tier mgechev architecture constraints of the Antigravity OS running tests against all directories, preventing systemic feature drift. |
| `task-manager` | Manage tasks and priorities. |
...
| `tutorial-engineer` | A team of AI agents designed to teach programming, concepts, and write tutorials. |
| `ui-ux-designer` | A team of multimodal agents for UI/UX design, visual analysis, and creative ideation. |
...
| `ux-collaborator` | Coordinate UX workstreams, research asks, and design debt. |
| `ux-researcher` | "Create refined user personas from research data — 3 personas with JTBD, pains, gains, and unexpected insights. Use when building personas from survey data, creating user profiles from research, or segmenting users for product decisions." |
| `centrifuge-protocol` | "Execute the full Centrifuge Protocol to keep the brain lean, private, and organized. Use as the cleanup engine behind the kit maintenance workflow when the user requests system optimization, task archiving, or hierarchical integrity auditing. Do NOT use to archive active code or configurations." |
...
| `visual-architect` | Expert in visualizing concepts through code. Generates Mermaid.js diagrams, ASCII art, and HTML/CSS prototypes. |
| `visual-processor` | Extract text and context from images for PM analysis. |
...
| `weekly-synth` | Weekly and monthly rollups with accomplishments and risks. |
...
| `youtube-summarizer` | Advanced agent for chatting with and summarizing YouTube videos. |
